...
import { getAllCartas, importCartas as apiImportCartas, deleteCarta as apiDeleteCarta } from '@/services/cartasApi'
...
const importCartas = async () => {
  ...
  try {
    const result = await apiImportCartas(previewData.value)
    
    if (result.success) {
      cartas.value = [...result.data, ...cartas.value]
      toast.add({ severity: 'success', summary: 'Éxito', detail: `${result.count} cartas importadas`, life: 3000 })
...
const deleteCarta = async () => {
  ...
  try {
    const result = await apiDeleteCarta(selectedCarta.value.id)
    if (result.success) {
...
const loadData = async () => {
  try {
    ...
    if (result.success) cartas.value = result.data
  } catch (error) {
