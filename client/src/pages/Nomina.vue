...
import { getAllNomina, createNomina, updateNomina, deleteNomina } from '@/services/nominaApi'
...
const saveItem = async () => {
  ...
  try {
    ...
    if (editingItem.value) {
      const result = await updateNomina(editingItem.value.id, data)
      if (result.success) {
        const index = gastos.value.findIndex(g => g.id === editingItem.value.id)
        if (index !== -1) gastos.value[index] = result.data
...
const deleteItem = async () => {
  ...
  deleting.value = true
  try {
    const result = await deleteNomina(itemToDelete.value.id)
    if (result.success) {
...
const loadData = async () => {
  loading.value = true
  try {
    const result = await getAllNomina()
    if (result.success) gastos.value = result.data
  } catch (error) {
